...
const BenchmarksTab = () => {
  ...
  if (loading) {
    return (
      ...
      >
        <Typography sx={{ fontSize: { xs: '0.9rem', sm: '1rem' } }}>Loading benchmarks...</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 3 } }}>
      ...
      <Grid container spacing={{ xs: 1.5, sm: 2 }} sx={{ mb: { xs: 2, sm: 3 } }}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Best WER" value={`${bestWER}%`} hint="Word Error Rate (lower is better)" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Best RTF" value={bestRTF.toFixed(2)} hint="Real Time Factor (lower is better)" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Best Cost" value={`$${bestCost.toFixed(2)}`} hint="Per 1000 minutes" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Models Tested" value={totalModels.toString()} hint="Available models" />
        </Grid>
      </Grid>
